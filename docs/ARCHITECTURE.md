...
`rg.ts` owns the matching engine boundary: argument arrays, workspace-scoped search roots, JSON match validation, matching-line counts, and exact occurrence ranges. It validates an absolute target for confinement, then gives both candidate enumeration and content search the same cwd-relative target so root-relative globs retain their intended meaning. One shared file-scope argument builder applies hidden-file, include, exclude, and ignore behavior to both phases. Git exclusions follow user globs so they cannot be overridden; explicit Git-internal search paths are rejected.
