...
- **宽泛搜索：** 首先返回按命中数排序的精确文件级摘要。
- **可复用展开：** 基于同一稳定内存快照分页文件摘要或匹配，并可反复选择一个或多个文件。
...
摘要行默认按命中数降序排列，同数时按路径排序，保证高噪音或高相关文件不会因为文件名字母靠后而被隐藏。如果仍有文件，可使用返回的 cursor 加 `mode="summary"` 继续分页。同一个原始摘要 cursor 可以从头读取详情、反复选择单个精确 `path`，或一次选择最多 20 个精确 `paths`，全程不重新扫描。带筛选条件的匹配 cursor 必须沿用原筛选集合；改变集合会明确失败，而不是静默漏掉前面的匹配。

每条匹配都会显示稳定的 `{match #N}` 标记。使用 `mode="inspect"`、摘要 cursor 和 `matchIndex=N`，即可直接检查这条保留匹配，无需再次拼接路径和行号。长匹配行会围绕 occurrence 截取，因此匹配文本始终可见，而列号仍是原行的绝对位置。
...
| ---------------------- | -----------------: | ----------------------: |
| 紧凑搜索：实际匹配     |                 33 |                      33 |
| 紧凑搜索：首次响应     |           898 字节 |          **1,347 字节** |
| 紧凑搜索：额外详情轮次 |             不需要 |              **不需要** |
| 宽泛搜索：实际匹配     | 达到限制后不可观察 |                 **233** |
| 宽泛搜索：首次显示详情 |             100 行 | **0（先返回精确摘要）** |
| 宽泛搜索：首次响应     |         9,728 字节 |            **315 字节** |
| 宽泛搜索：首次响应缩减 |                  — |               **96.8%** |
...
| ---------- | ---------: | --------------: | -----------: | ---------: |
| `full`     |        80% |           2,000 |           18 | 4,787 字节 |
| `tight`    |        30% |           1,000 |  0（先摘要） |   327 字节 |
| `critical` |         8% |             500 |  0（先摘要） |   328 字节 |
...
- `summary`：返回按命中数排序的文件摘要页；仍有文件时，使用返回的 cursor 和 `mode="summary"` 继续分页。
- `matches`：立即返回第一页自适应预算详情。
- `inspect`：检查 `path`/`line` 所在的最小可用代码符号，或结合 cursor 与 `matchIndex` 直接检查保留匹配；cursor 检查会拒绝变化后的源文件。
...
2. 匹配 cursor 保持快照顺序，分页不得重复或遗漏；带文件筛选的匹配 cursor 会拒绝改变筛选集合。
3. 文件摘要按命中数降序排列，同数按路径排序，并可在不重新搜索的前提下分页覆盖全部保留文件。
4. 已保留的匹配行文本和 occurrence 范围保持快照稳定；文件 revision 变化后，当前文件上下文会明确省略，不与旧匹配混合。
5. 超过 50,000 条保留上限时，文本和结构化详情都必须标记为 `partial`。
6. 隐式 `auto` 详情试装会依据上报的上下文余量，以约 2,000、1,000 或 500 个估算结果文本 Token 为目标；显式请求和续页保留 2,000 Token 目标。所有页面仍受 100 条匹配和 16 KiB 硬上限约束。
7. 超过 500 字符的匹配行会围绕 occurrence 截取并在详情中计数；匹配文本保持可见，列号仍为原行绝对位置。
8. 大于 5 MiB、无法读取或没有可用结构 provider 的文件会省略结构/上下文并明确报告。
9. `mode=inspect` 只有在 provider 能证明范围时才返回 enclosing symbol。超大范围仍受字节限制且围绕请求行居中；cursor 检查会拒绝变化后的源文件。
10. cursor 无效、过期、筛选不一致或已耗尽，结构请求无效，匹配编号未保留，以及子进程失败等情况必须报错，不能伪装成空搜索成功。
...
`mode=inspect` 接受相对于工作目录的 `path` 和 1 起始 `line`，也可接受摘要 cursor 与可见的 1 起始 `matchIndex`。它返回有界、带行号的源代码范围；如果可用 provider 能识别最小 enclosing symbol，还会返回其名称、类型和范围。超大符号会围绕请求行居中。cursor 检查在源 revision 变化时明确失败。

基础定位能力适用于所有文本语言，因为它来自 ripgrep 的匹配范围。符号检查采用能力协商：只有 Universal Ctags 接受运行时实际使用的 JSON 输出、line/end 字段和 extras 参数时，默认 provider 才会启用。该依赖可选且不会自动下载。不能提供可靠结束范围的语言 parser 返回 `no-symbol`，不会猜测代码块。provider 缺失、解析失败、文件过大、源变化和文件不可读都会体现在 `details.structure.status` 中。

搜索页面会合并重叠上下文，并显示稳定的 `{match #N}` 标记。有效 UTF-8 文本的匹配列使用 UTF-16 位置；带 `b` 后缀的范围是非 UTF-8 数据的原始字节偏移。长匹配行围绕 occurrence 截取。只有保留 revision 仍匹配时才读取周边上下文，否则会省略并明确报告。原始 ripgrep 或 Ctags 协议输出不会发送给模型。
